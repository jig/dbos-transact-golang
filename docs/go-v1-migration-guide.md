...
- **A client no longer creates or migrates the system database** (matching the Python and TypeScript clients). It verifies the schema instead: `NewClient` fails if the system database is missing or behind the version the client's build requires. Migrate it from the application (`Launch`) or out of band with `dbos migrate`.
...
- `Config.SkipMigrations` verifies the system database schema on launch instead of creating and migrating it — for a process whose database role cannot run DDL, or a deployment that migrates out of band with `dbos migrate`. Launch then fails if the system database is missing or behind the version the build requires. Equivalent to Python's `run_migrations` and TypeScript's `runMigrations`, inverted so the Go zero value keeps migrating.
