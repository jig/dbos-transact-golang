...
- **Last upstream incorporated:** `e7cc54c` — "consolidate retry logic (#357)".
  Everything below is `git diff e7cc54c..HEAD`.
- **Module path is unchanged:** `go.mod` is still
  `module github.com/dbos-inc/dbos-transact-golang`. The fork is *not* renamed to
  `jig/...`; consumers (e.g. fluxos6) depend on it via a `replace` directive. A
  fresh fork can keep this (patch + replace) or rename the module to its own
  namespace — decide once.
- **PoC stance:** no PL/pgSQL ever; drop & recreate the schema (no data
  migration). This is a deliberate PoC stance.
- Total delta at time of writing: ~23 source files, +3k/-1k lines, plus tests.
