...
// outside the retry loop, and every attempt re-runs the same logical operation
// with them. When the allocation instead happened inside the retried call, a
// transient failure (e.g. SQLITE_BUSY) leaked IDs and recorded a gapped
// history, and a replay re-executed inside the gap (observed as a fluxos8
// golden-history divergence).
//
...
// the step IDs each attempt carried; every attempt must carry the same,
// pre-assigned IDs, and the operation must be recorded at step 0.
...
// simulatedNetError is matched by postgresDialect.IsRetryable's net.Error
// branch, which is part of the default retry condition chain on any backend.
...
type flakyStepSysDB struct {
	...
	mu       sync.Mutex
	failLeft map[string]int     // op -> remaining injected failures
	seen     map[string][][]int // op -> per-attempt carried step IDs (nil entry = ID missing)
}

func newFlakySysDB(real systemDatabase) *flakyStepSysDB {
	return &flakyStepSysDB{
		systemDatabase: real,
...
// observe records the IDs an attempt carried and reports whether to fail it.
func (f *flakyStepSysDB) observe(op string, ids ...*int) bool {
	f.mu.Lock()
	defer f.mu.Unlock()
	attempt := make([]int, 0, len(ids))
	for _, id := range ids {
		if id == nil {
			attempt = nil
			break
	...
	f.seen[op] = append(f.seen[op], attempt)
	if f.failLeft[op] > 0 {
		f.failLeft[op]--
		return true
	}
...
// attempts returns the recorded per-attempt IDs for op.
...
func (f *flakyStepSysDB) getEvent(ctx context.Context, input getEventInput) (*getEventResult, error) {
	if f.observe("getEvent", input.stepID, input.sleepStepID) {
		return nil, simulatedNetError{}
	}
...
// requireStableIDs asserts every attempt carried the same pre-assigned step
// IDs: a nil entry means an attempt arrived without IDs (the allocation
// happened inside the retried call — the leak this test guards against).
func requireStableIDs(t *testing.T, attempts [][]int, wantAttempts int) {
	t.Helper()
	require.Len(t, attempts, wantAttempts)
	for i, a := range attempts {
		require.NotNil(t, a, "attempt %d carried no pre-assigned step IDs (allocated inside the retry loop)", i)
		require.Equal(t, attempts[0], a, "attempt %d used different step IDs than attempt 0", i)
	}
}

// requireStepAtID asserts that the workflow recorded functionName at stepID —
// i.e. the transient retries did not shift it by leaking IDs.
