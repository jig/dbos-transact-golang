...
// recorded error string of the form "DBOS Error <code>: <message>", so that a
// legacy plain-string recording (pre-gob histories) replays with the same type
// and code the first execution saw (e.g. a TimeoutError the workflow branches
// on). Anything else is returned as a plain error. Fork §5: new recordings are
// gob-encoded and type-preserving; this covers histories recorded before that.
